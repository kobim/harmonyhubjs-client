...
function getIdentity (hubhost, hubport) {
  ...
  xmppClient.on('error', function (e) {
    debug('XMPP client error:', e, iqId ? iqId.toString() : 'NO IQID')
    if (e.message.indexOf('MAX_CLIENTS') !== -1) {
      debug('Too many connections error, bailing out')
      failure()
    }
  })
...
function loginWithIdentity (identity, hubhost, hubport) {
  ...
  xmppClient.once('error', function (e) {
    debug('XMPP client error during loginWithIdentity', e)
    if (e.message.indexOf('MAX_CLIENTS') !== -1) {
      deferred.reject(new Error('Couldn\'t login to XMPP'))
    }
  })
